...
- **Descriptive:** It should explain _what_ the value is (e.g., `daysSinceCreation` instead of `d`).
...
- **Misinterpretation:** A function named `check()` is dangerous. Does it just _check_ something? Or does it _check and delete_ invalid items? Ambiguity leads to bugs.
